...
export function updateTasks() {
    ...
    .then(tasks => {
        ...
        if(doneTasks.length > 0) {
            taskList.insertAdjacentHTML('beforeend', `
                <h1 class="mb-4 text-3xl font-extrabold text-gray-900 dark:text-white">
                    <span class="text-transparent bg-clip-text bg-gradient-to-r to-primary from-highlight">Done: </span>
                </h1>
            `);
    ...
    taskElement.innerHTML = `
        ...
        <div class="content ml-4 mt-1 flex flex-col" style="flex: 0 55%;">
            <div class="task-header mr-4">
                <h3 class="task-title font-bold mb-1 ${task.status == 1 ? 'line-through' : ''}" style="word-break: break-word; overflow-wrap: break-word;">${task.name}</h3>
            </div>
            <div class="description-container">
                <p class="task-description mb-1 ${task.status == 1 ? 'line-through' : ''} " style="word-break: break-word; overflow-wrap: break-word;">${task.description}</p>
            </div>
        </div>

        <div class="date-location-container flex flex-col justify-center items-end" style="flex: 0 25%;">
            <div class="text-right" style="width: 100%;">
                <p class="task-goaldate text-gray-600 text-sm">${goalDateString}</p>
            </div>
            ${taskLocation}
        </div>
